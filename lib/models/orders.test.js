  ...
  describe("Order schema", function() {
    ...
    describe("Instance methods", function() {
      describe("prepare", function() {
        it("should convert strings to hex", async function() {
          const orderObj = JSON.parse(JSON.stringify(validOrderObj));
          const order = await model.create(orderObj);
          const prepared = order.prepareOrder();

          expect(Web3.utils.isHexStrict(prepared[0])).to.be.true;
          expect(Web3.utils.isHexStrict(prepared[1])).to.be.true;
          expect(Web3.utils.isHexStrict(prepared[2])).to.be.true;
        });

        it("should include the withdrawAddress in last index", async function() {
          const orderObj = JSON.parse(JSON.stringify(validOrderObj));
          const order = await model.create(orderObj);
          const prepared = order.prepareOrder();

          expect(eth.isETHAddress(prepared[prepared.length - 1])).to.be.true;
        });
      });

      describe("sendOrder", function() {
        it("should be able to call without crashing", async function() {
          const orderObj = JSON.parse(JSON.stringify(validOrderObj));
          const order = await model.create(orderObj);
          const response = await order.sendOrder();

          expect(response).to.exist;
          expect(response.status).to.equal("0x01");
        });
      });
    });
    ...
    describe("Invalid orders", function() {
      ...
      describe("positionDuration", function() {
        ...
        it("should not allow too big", async function() {
          const orderObj = JSON.parse(JSON.stringify(validOrderObj));
          orderObj[field] =
            config.get("constants.order.positionDurationMax") + 1;
